"""
    ...
    1D electrostatic particle-in-cell solver for relativistic two-stream instability.
...
for it in xrange(NT+1):
    ...
    bins,edges=np.histogram(gamma,bins=80,range=(-3.2,3.2))
    ...
    if (np.mod(it, TOut) == 0) and verbose:
        ...
        plt.plot(vc,fv, label='f($\gamma$)', linewidth=2)
...
np.savetxt('r_field_ene.txt',(t,histPotE))
